...
//added by Nick Kasprak 7/7/14 to deal with Richard Kogan sorting issue
function last_name_custom_sort($a, $b) {
	return strcmp($a[1],$b[1]);
}

function output_active_authors_by_last_name() {
	
	$users = get_users();
	$sortArray = array();
	foreach ($users as $user) {
		if ($user->last_name) {
			array_push($sortArray,array($user->ID,$user->last_name,$user->display_name));
			
		}
	}
	usort($sortArray,'last_name_custom_sort');
	for ($authorIndex = 0;$authorIndex < count($sortArray);$authorIndex++ ) {
		$user = $sortArray[$authorIndex];
		$id = $user[0];
		if (count_user_posts($id) > 0) {
			echo '<li><a href="'.get_author_posts_url($id) . '">'.$user[2].'</li>';
		}
	}
} 
